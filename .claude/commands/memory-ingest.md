...
First, let me check if the service is running:

```bash
# Check if the service is running
curl -s http://127.0.0.1:8000/api/health || echo "Service not running"
```

Then I'll upload the document:
...
curl -s "http://127.0.0.1:8000/api/documents/status/UPLOAD_ID"
```
...
2. **Verify service availability** at `http://127.0.0.1:8000/`
